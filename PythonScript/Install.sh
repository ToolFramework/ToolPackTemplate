...
if [ ${REDHATLIKE} -eq 0 ]; then
	...
	echo "gcc-c++ gcc make cmake git python3 python3-libs python3-devel python3-pip which"
...
# make dependencies dir it if it doesn't exist...
...
if [ -d cppyy-backend ]; then
	echo "${DEPENDENCIESDIR}/cppyy-backend already exists! Overwrite it?"
	select result in Yes No; do
		if [ "$result" == "Yes" ]; then
			rm -rf cppyy-backend
			break;
...
cd ${DEPENDENCIESDIR}
if [ -d CPyCppyy ]; then
	echo "${DEPENDENCIESDIR}/CPyCppyy already exists! Overwrite it?"
	select result in Yes No; do
		if [ "$result" == "Yes" ]; then
			rm -rf ${DEPENDENCIESDIR}/CPyCppyy
			break;
		elif [ "$result" == "No" ]; then
			break;
		else
...
cd ${DEPENDENCIESDIR}
if [ -d cppyy ]; then
	echo "${DEPENDENCIESDIR}/cppyy already exists! Overwrite it?"
	select result in Yes No; do
		if [ "$result" == "Yes" ]; then
			rm -rf ${DEPENDENCIESDIR}/cppyy
			break;
		elif [ "$result" == "No" ]; then
			break;
		else
